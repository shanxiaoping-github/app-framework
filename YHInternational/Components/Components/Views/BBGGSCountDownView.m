...
@interface BBGGSCountDownView ()<BBGCountdownObserver>
...
@implementation BBGGSCountDownView
...
- (void)countdown {
    if (self.status == 0) {
        ...
        if (self.countdownTime > 0) {
            if (self.allTime > self.countdownTime) {
                NSString *monthStr = [NSString dateWithTimeInterval:self.currentTime formatter:@"MM"];
                NSString *dayStr = [NSString dateWithTimeInterval:self.currentTime formatter:@"dd"];
                NSString *hourStr = [NSString dateWithTimeInterval:self.currentTime formatter:@"HH:mm"];
                self.timeLab.text = [NSString stringWithFormat:@"%@月%@日 %@",monthStr,dayStr,hourStr];
                ...
                self.imageView.frame = CGRectMake(self.timeLab.frame.origin.x-24.0f, self.timeLab.frame.origin.y-2.0f, 20.0f, 20.0f);
            ...
            }else if (self.allTime>=0&&self.allTime<=self.countdownTime) {
                ...
                if ([self.delegate conformsToProtocol:@protocol(BBGGSCountDownViewDelegate)]&&[self.delegate respondsToSelector:@selector(countDownFinish:)]) {
                    [self.delegate countDownFinish:0];
                }
        ...
        if (self.allTime >= 0) {
            ...
            self.hidden = YES;
            if ([self.delegate conformsToProtocol:@protocol(BBGGSCountDownViewDelegate)]&&[self.delegate respondsToSelector:@selector(countDownFinish:)]) {
                [self.delegate countDownFinish:2];
            }
            [[BBGCountdownManager sharedInstance]removeObserver:self];
        }
    }
