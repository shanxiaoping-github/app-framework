...
@implementation BBGUploadUserHeadRequest
...
- (void)uploadUserHeaderImg:(void (^)(AFHTTPRequestOperation *operation, BBGResponse * responseData))success failure:(void (^)(AFHTTPRequestOperation *operation, NSString * errorMsg, NSString * errorCode))failure {
    ...
    [manager POST:urlStr parameters:[self parametersDict] constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        [formData appendPartWithFileData:_headImgData name:@"Filedata" fileName:@"header.jpg" mimeType:@"image/jpg"];
...
- (Class)responseClass{
    return [BBGUploadUserHeadResponse class];
}
