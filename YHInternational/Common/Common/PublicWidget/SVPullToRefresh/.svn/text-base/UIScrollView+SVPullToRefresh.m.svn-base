...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        BBGRefreshView * refreshView;
        
        refreshView = [[BBGRefreshView alloc]initWithFrame:CGRectMake(0, 0, 200, 81)];
        [view setCustomView:refreshView forState:SVPullToRefreshStateAll];
...
- (void)triggerPullToRefresh {
    ...
    [self.pullToRefreshView.refreshView startSpin];
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        self.refreshState = RefreshStateFirst;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGRect titleRect = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth, self.titleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleLabel.font} context:NULL];
        CGSize titleSize = titleRect.size;
        
        CGRect subtitleRect = [self.subtitleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth, self.subtitleLabel.font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.subtitleLabel.font} context:NULL];
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == SVPullToRefreshStateStopped && self.position == SVPullToRefreshPositionTop && fabs(contentOffset.y) > SVPullToRefreshViewHeight){
...
- (void)stopAnimating:(void (^)(void))actionHandler {
    self.isAnimating = NO;
    switch (self.position) {
        case SVPullToRefreshPositionTop:
        {
            NSLog(@"这里是状态：%lu",(unsigned long)self.scrollView.pullToRefreshView.state);
            if (self.scrollView.pullToRefreshView.state == SVPullToRefreshStateLoading) {
                self.refreshState = RefreshStateStopped;
                ...
                if (self.refreshState == RefreshStateFirst) {
                    self.refreshState = RefreshStateStopped;
                    if (actionHandler) {
                        actionHandler();
                    }
            ...
            [self.scrollView.pullToRefreshView.refreshView stopSpin];
            [self.scrollView.pullToRefreshView.refreshView removeFromSuperview];
            self.scrollView.pullToRefreshView.refreshView = nil;
            
            [self.scrollView setContentOffset:CGPointMake(0, 0) animated:YES];
            BBGRefreshView * refreshView = [[BBGRefreshView alloc]initWithFrame:CGRectMake(0, 0, 200, 81)];
            [self.scrollView.pullToRefreshView setCustomView:refreshView forState:SVPullToRefreshStateAll];
            self.scrollView.pullToRefreshView.refreshView = refreshView;
